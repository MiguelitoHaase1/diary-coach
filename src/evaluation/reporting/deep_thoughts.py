...
class DeepThoughtsGenerator:
    ...
    def __init__(
        self,
        llm_service: Optional[AnthropicService] = None,
        tier: LLMTier = LLMTier.STANDARD,
    ):
        """Initialize the Deep Thoughts Generator.

        Args:
            llm_service: Optional LLM service. If not provided, creates
                service based on tier.
            tier: LLM tier to use (CHEAP, STANDARD, PREMIUM, or O3) -
                defaults to STANDARD (Claude Sonnet 4)
        """
        if llm_service:
