...
class EvaluationReport:
    ...
    def to_markdown(self) -> str:
        ...
        median_ms = (
            int(sum(self.response_times_ms) / len(self.response_times_ms))
        ...
        p80_status = (
            ...
            else "⚠️" if self.percentile_80 < 2000 else "❌"
        )
        under_1s_status = (
            "✅"
            if self.responses_under_1s_percentage >= 0.8
            else "⚠️" if self.responses_under_1s_percentage >= 0.6 else "❌"
        )

        report_id = self.conversation_metadata.get('report_id', '1')
        markdown = f"""# Coaching Evaluation Report #{report_id}
        ...
        for score in self.behavioral_scores:
            if score.value < 0.5:
                if score.analyzer_name == "SpecificityPush":
                    suggestions.append(
                        "Challenge vague statements like 'be productive' "
                        "with specific follow-up questions"
                    )
                elif score.analyzer_name == "ActionOrientation":
                    suggestions.append(
                        "Drive toward concrete commitments and next steps"
                    )
                    ...
                    suggestions.append(
                        "Question systematic thinking patterns more directly"
                    )
...
class EvaluationReporter:
    ...
    async def generate_report(
        ...
        overall_score = self._calculate_overall_score(
            behavioral_scores, performance_data
        )
        ...
        response_times_ms = (
            performance_data.get("response_times_ms", []) if performance_data else []
        )
        percentile_80 = (
            performance_data.get("percentile_80", 0) if performance_data else 0
        )
        responses_under_1s_percentage = (
            performance_data.get("responses_under_1s_percentage", 0)
            if performance_data
            else 0
        )
    ...
    async def generate_light_report(
        ...
        # Run actual behavioral analysis (same as full report)
        behavioral_scores = []
        user_messages = [msg for msg in conversation.messages if msg["role"] == "user"]

        for analyzer in analyzers:
            if coach_messages:
                # Analyze first coach response with proper context
                i = 0  # First coach message
                coach_msg = coach_messages[i]
        ...
        # Generate brief AI reflection based on actual scores
        ai_reflection = self._generate_brief_reflection(behavioral_scores, conversation)
    ...
    def _calculate_overall_score(
        ...
        behavioral_avg = sum(score.value for score in behavioral_scores) / len(
            behavioral_scores
        )
    ...
    def _generate_ai_reflection(
        ...
        reflection = (
            f"Based on this conversation with a {conversation.persona_type} persona, "
        )

        if strongest.value > 0.7:
            name = strongest.analyzer_name.lower()
            score = strongest.value * 10
            reflection += (
                f"I performed well in {name}, achieving a score of {score:.1f}/10. "
            )
        ...
        if weakest.value < 0.5:
            ...
            score = weakest.value * 10
            reflection += (
                f"I could strengthen my {name}, which scored {score:.1f}/10. "
            )
        ...
        if conversation.persona_type == "FrameworkRigid":
            if conversation.breakthrough_achieved:
                reflection += (
                    "I successfully broke through the framework-rigid "
                    "resistance patterns by challenging systematic thinking."
                )
            else:
                reflection += (
                    "I could push harder against the systematic thinking "
                    "patterns to create breakthrough moments."
                )
        elif conversation.persona_type == "ControlFreak":
            reflection += (
                "Perfectionist resistance requires patient emotional "
                "presence combined with direct action orientation."
            )
        elif conversation.persona_type == "LegacyBuilder":
            reflection += (
                "Future-focused deflection needs stronger present-moment "
                "anchoring to drive immediate action."
            )
        elif conversation.persona_type == "Real User":
            reflection += (
                "Real user conversations require balancing genuine "
                "responsiveness with coaching effectiveness."
            )

        # Add overall reflection
        avg_score = sum(score.value for score in behavioral_scores) / len(
            behavioral_scores
        )
        if avg_score > 0.8:
            reflection += (
                " Overall, this was a highly effective coaching "
                "conversation."
            )
        elif avg_score > 0.6:
            reflection += (
                " Overall, this was a moderately effective coaching "
                "conversation with room for improvement."
            )
        else:
            reflection += (
                " Overall, this conversation indicates significant "
                "opportunities for coaching improvement."
            )
    ...
    def _generate_brief_reflection(
        ...
        """Generate brief AI reflection for light report.
        ...
        Returns:
            Brief AI reflection text
        """
        if not behavioral_scores:
            return (
                "No behavioral analysis available. Use 'deep report' for "
        ...
        # Build brief reflection
        reflection = f"Quick Analysis: {avg_score*10:.1f}/10 overall. "
        reflection += (
            f"Strongest: {strongest.analyzer_name} ({strongest.value*10:.1f}/10). "
        )
        reflection += (
            f"Needs work: {weakest.analyzer_name} ({weakest.value*10:.1f}/10). "
        )
        reflection += "Use 'deep report' for detailed AI reflection and analysis."
    ...
    async def generate_deep_report(
        ...
        ai_reflection = await self._generate_deep_ai_reflection(
            behavioral_scores, conversation, user_notes
        )
    ...
    async def _generate_deep_ai_reflection(
        ...
        behavioral_summary = "\n".join(
            [
                f"- {score.analyzer_name}: {score.value*10:.1f}/10 - "
                f"{score.reasoning}"
        ...
        reflection_prompt = f"""You are an expert coaching evaluator. Analyze this
coaching conversation and provide a comprehensive reflection.
...
Write in first person as the AI coach reflecting on performance.
Be specific and actionable."""
        ...
        try:
            ...
            # Fallback to simple reflection
            simple_reflection = self._generate_ai_reflection(
                behavioral_scores, conversation
            )
            return f"Deep reflection failed: {str(e)}. {simple_reflection}"
