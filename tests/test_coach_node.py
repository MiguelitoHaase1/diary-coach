...
async def test_coach_node_preserves_behavior():
    ...
    class MockLLMService:
        async def generate_response(self, messages, system_prompt,
            ...
            return ("Good morning Michael! What dragon are you most "
                    "excited to slay today?")
...
async def test_coach_node_error_handling():
    ...
    # Should have added an error response
    responses = result_state.get_agent_responses()
    assert len(responses) == 1
    assert "trouble processing" in responses[0]["content"].lower()
